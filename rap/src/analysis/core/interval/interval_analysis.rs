...
impl<'tcx> MirPass<'tcx> for DataflowConstProp {
    ...
    fn run_pass(&self, tcx: TyCtxt<'tcx>, body: &mut Body<'tcx>) {
        ...
        let place_limit = if tcx.sess.mir_opt_level() < 4 {
            Some(PLACE_LIMIT)
        } else {
            None
        };
        ...
        let analysis = IntrervalAnalysis::new(tcx, body, map);
        let mut _results = debug_span!("analyze")
...
struct IntrervalAnalysis<'a, 'tcx> {
    map: Map,
    tcx: TyCtxt<'tcx>,
...
impl<'tcx> ValueAnalysis<'tcx> for IntrervalAnalysis<'_, 'tcx> {
    type Value = FlatSet<Scalar>;

    const NAME: &'static str = "IntrervalAnalysis";
    ...
    fn handle_assign(
        ...
        match rvalue {
                ...
                let (variant_target, variant_index) = match **kind {
                    ...
                    AggregateKind::Adt(def_id, variant_index, ..) => {
                        match self.tcx.def_kind(def_id) {
                            ...
                            DefKind::Enum => (
                                self.map
                                    .apply(target_idx, TrackElem::Variant(variant_index)),
                                Some(variant_index),
                            ),
                            _ => return,
                        }
                ...
                if let Some(variant_target_idx) = variant_target {
                    for (field_index, operand) in operands.iter_enumerated() {
                        if let Some(field) = self
                            .map()
                            .apply(variant_target_idx, TrackElem::Field(field_index))
                        {
            ...
            Rvalue::BinaryOp(op, box (left, right)) if op.is_overflowing() => {
                ...
                let Some(target) = self.map().find(target.as_ref()) else {
                    return;
                };
...
impl<'a, 'tcx> IntrervalAnalysis<'a, 'tcx> {
    ...
    fn assign_constant(
        ...
        for &(mut proj_elem) in projection {
            if let PlaceElem::Index(index) = proj_elem {
                ...
                {
                    proj_elem = PlaceElem::ConstantIndex {
                        offset,
                        min_length,
                        from_end: false,
                    };
...
// impl<'mir, 'tcx>
//     ResultsVisitor<'mir, 'tcx, Results<'tcx, ValueAnalysisWrapper<IntrervalAnalysis<'_, 'tcx>>>>
//     for Collector<'tcx, '_>
// {
...
//         &mut self,
//         results: &mut Results<'tcx, ValueAnalysisWrapper<IntrervalAnalysis<'_, 'tcx>>>,
